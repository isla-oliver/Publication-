...
fig1b=ggplot(boh%>%filter(Location=='SR'))+
  geom_histogram(col = 'black', fill = "#C71000B2", aes(x=Length), binwidth = 65)+
  ylim(0,60)+xlim(200,900)+
...
fig3b=ggplot(boh%>%filter(Location=='Chagos '))+
  geom_histogram(col='black', fill = "#FF6F00B2", aes(x=Length), binwidth = 65)+
  ylim(0,60)+xlim(200,900)+
  theme_classic()+theme(axis.title.y = element_blank())+
...
fig2 + ggtitle(expression(italic("L. bohar")))

fig2N <- annotate_figure(fig2, 
                fig.lab = "L. bohar", fig.lab.face = "italic", fig.lab.size = 16)
fig2N
